...
cleanup() {
    echo
    echo "Shutting down main services and infrastructure..."
    kill_local_services
    docker compose -f docker-compose.infra.yml -p involo down
    exit 0
}
...
# Chromium is only required for the real Instagram Playwright scraper, tracking, or PDF
# rendering. Fixture/fake providers need no browser.
...
start_public_tunnel() {
    ...
    if [ -z "$public_url" ]; then
        echo "Failed to start localtunnel. See logs/localtunnel.log for details."
        exit 1
    fi
...
echo "  MinIO:     http://localhost:8027"
if [ -n "${INVOLO_NGROK_DOMAIN:-}" ]; then
